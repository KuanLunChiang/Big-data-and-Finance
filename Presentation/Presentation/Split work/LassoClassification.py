...
def parallel_LASSO (name, regress , windowList, paramList, fixed):
    ...
    se, rmse, wsize, para, prdList = sequential_grid_tune( dataDict[i],lasso, window = windowList, paramList = paramList, paramName = 'alpha', startPara = 0.001, regress = regress, fixed = fixed)
...
errorList_lassoc, lassoc_wisize = multi_proccess_LASSO(windowList = np.arange(60,90), paramList = np.arange(0.001,0.01,0.001),regress = False)
SSED_lassoc, lassoc_tuned = multi_proccess_LASSO_report(errorList_lassoc, lassoc_wisize, isClassif = True)
plot_differential_report(colName = fxspot.drop("Date", axis = 1).columns.tolist(), data = SSED_lassoc, para = 'SSEDif', row = 3, col = 3, supT = 'LASSO SSE Differential')
plot_differential_report(colName = fxspot.drop("Date", axis = 1).columns.tolist(), data = SSED_lassoc, para = 'oosrsquare', row = 3, col = 3, supT = 'LASSO OOS R-Squared')

new_excel(storepath)
[excel_writer(storepath,('SSED_'+i),SSED_lasso[i])for i in fxspot.drop('Date',axis =1).columns.tolist()]
[excel_writer(storepath,('prd_'+i),pd.Series(lasso_prd[i]))for i in fxspot.drop('Date',axis =1).columns.tolist()]
