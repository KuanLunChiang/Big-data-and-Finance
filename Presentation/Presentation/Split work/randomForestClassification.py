...
def parallel_LASSO (name, regress , windowList, paramList, fixed):
    ...
    se, rmse, wsize, para, prdList = sequential_grid_tune( dataDict[i],lasso, window = windowList, paramList = paramList, paramName = 'alpha', startPara = 0.01, regress = regress, fixed = fixed)
...
rfc_SSED, rfc_winSize_currency = results_report(mdl = rf,data = dataDict,isTuned = True, isClassif = True, func = sequential_grid_tune,arg = (rf, np.arange(10,11,1),np.arange(2,4), 'max_depth', 2, False,False))
...
[excel_writer(storepath,('prd_'+i),rf_prd[i])for i in fxspot.drop('Date',axis =1).columns.tolist()]
